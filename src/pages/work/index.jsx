...
const Work = ({ allWorks, allTags }) => {
     ...
     useEffect(()=>{
          if(tag){
               const filter = allWorks.filter((f)=>f.tags.some((t)=>t.slug.current === tag))
               setWorks(filter)
          }
     },[tag])
          ...
          <>
               ...
               <Layout>
                    ...
                    <ul className='flex items-center gap-3 font-light flex-wrap  xl:container mx-auto'>
                         <li className='text-gray hover:cursor-pointer hover:text-main uppercase' onClick={()=>setWorks(allWorks)}>All</li>
                         {
                              allTags.map((f, id) => (
                                   <li key={id} className='text-gray hover:cursor-pointer hover:text-main uppercase' onClick={()=>setTag(f.slug.current)}>{f.name}</li>
                              ))
                         }
                    </ul>
